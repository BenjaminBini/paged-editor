...
const LOGO_PRESETS: Record<string, { label: string; file: string }[]> = {
...
// Editor-bundled preset logos — served from the editor's static root, not the workspace.
const BUNDLED_LOGO_FILES: Set<string> = new Set(
  Object.values(LOGO_PRESETS).flat().map((p: { file: string }) => p.file),
);
...
let _patchPreview: ((data: any) => any) | null = null;

export function setOnRender(fn: () => void): void {
  _onRender = fn;
}

export function setOnDirty(fn: () => void): void {
  _onDirty = fn;
}

export function setOnPatchPreview(fn: (data: any) => any): void {
  _patchPreview = fn;
}
...
export function showCoverForm(): void {
  ensureFormDom();
  if (!container) return;
...
export function hideCoverForm(): void {
  if (container) container.classList.remove("visible");
  const cmWrapper = cm.getWrapperElement();
  if (cmWrapper) cmWrapper.style.display = "";
}

export function isCoverFormVisible(): boolean {
  return container !== null && container.classList.contains("visible");
}

export function populateCoverForm(jsonString: string): void {
  ensureFormDom();
  let data;
  try {
...
export function serializeCoverForm(): string {
  const data: {
    title: string;
    doctype: string;
    ref: string;
...
export function syncCoverFormToEditor(): void {
...
function ensureFormDom(): void {
  if (formBuilt) return;
  buildCoverFormDom();
  formBuilt = true;
}

function buildCoverFormDom(): void {
...
function createTextField(labelText: string, placeholder: string): HTMLInputElement {
...
function createTextarea(labelText: string, placeholder: string, rows: number): HTMLTextAreaElement {
  const field = document.createElement("div");
  field.className = "cover-form-field";
  const label = document.createElement("label");
  label.textContent = labelText;
...
function createCheckbox(labelText: string): HTMLLabelElement {
...
function buildLogoCard(key: string): HTMLDivElement {
  ...
  fileInput.addEventListener("change", () => {
    ...
    reader.onload = () => {
      ...
      writeBinaryFile(destPath, base64 as unknown as ArrayBuffer).catch((e) => {
...
function buildSliderRow(labelText: string, min: number, max: number, step: number, defaultValue: number): { row: HTMLDivElement; input: HTMLInputElement; valueEl: HTMLSpanElement } {
  const row = document.createElement("div");
...
function syncSourceToggle(key: string): void {
  const r = refs.logos[key];
  ...
  const presets = LOGO_PRESETS[key] || [];
  const matchedPreset = !r._dataUrl && presets.find((p: { label: string; file: string }) => p.file === r.file);
  card.querySelectorAll(".logo-source-btn").forEach((btn: HTMLElement) => {
...
async function updateLogoPreview(key: string): Promise<void> {
  const r = refs.logos[key];
  const raw: string = r._dataUrl || r.file;
  if (raw) {
    // Data URLs (from fresh upload) are used directly; relative paths need
    // the workspace asset base so they resolve to the workspace folder.
    let src: string = raw;
    if (!raw.startsWith("data:") && !BUNDLED_LOGO_FILES.has(raw)) {
      const base: string = await getAssetBaseHref(getFolderPath() || "");
      if (base) {
        try { src = new URL(raw, base).href; } catch { /* keep raw */ }
      }
    }
    r.img.src = src;
    r.img.style.display = "";
...
function updateSliderValues(key: string): void {
  const r = refs.logos[key];
...
function toggleSliders(key: string): void {
  const r = refs.logos[key];
...
function scheduleSyncToEditor(): void {
