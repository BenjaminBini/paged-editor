...
import { pagedReady } from "../document/rendering/preview-renderer.js";
import {
  openPreviewTab,
  previewPdf,
  previewFullMemoire,
  closePdfPanel,
  savePdfAs,
} from "../document/export/pdf-export-service.js";
import {
  triggerRender,
  scalePreview,
  getPreviewFrame,
...
import {
  ...
  setupScrollSync,
  rebuildAnchorMap,
} from "../document/sync/preview-sync-setup.js";
import {
  refreshTableWidgets,
  insertTable,
  getTableRangeAt,
...
import { closeDiffModal, resolveConflict } from "../workspace/files/diff-merge-service.js";
import {
...
} from '../collaboration/chat-sidebar-controller.js';
import { buildOutline, updateOutlineHighlight } from "../editor/outline-manager.js";
import { tryRestore, buildRecentUI } from "./session-restore.js";
import { setupKeyboardShortcuts, wireElectronMenus } from "./ui/keyboard-shortcuts.js";
import {
  hideWelcome, showWelcome, initMenubar,
  updateMenuState as _updateMenuState,
} from "./ui/menu-state-manager.js";
import {
  wireTabCallbacks, wireSidebarCallbacks,
  onChangeDirtyTracking,
} from "./tab-integration.js";
import {
...
// ── Tab lifecycle & sidebar wiring (delegated to tab-integration.js) ────────────

function detachCmListeners(): void {
...
function reattachCmListeners(): void {
...
function isEditableMarkdownContext(): boolean {
...
busOn("content-loaded", () => {
  ...
  for (const _tw of tableWidgets.values()) destroyTableWidget();
...
let _scaleTimer: ReturnType<typeof setTimeout> | null = null;
let _anchorTimer: ReturnType<typeof setTimeout> | null = null;
busOn("section-ready", () => {
  const frame = getPreviewFrame();
  if (frame && isEditableMarkdownContext()) setupPreviewClick();
  clearTimeout(_scaleTimer ?? undefined);
  clearTimeout(_anchorTimer ?? undefined);
...
let _anchorRebuildTimer: ReturnType<typeof setTimeout> | null = null;
...
let twRefreshTimer: ReturnType<typeof setTimeout> | null = null;

function onChangeAutoRender(): void {
  ...
  if (!twSyncing && isEditableMarkdownContext()) {
    setTableRangesDirty();
    clearTimeout(twRefreshTimer ?? undefined);
    twRefreshTimer = setTimeout(refreshTableWidgets, 200);
  }
}
cm.on("change", onChangeAutoRender);

// ── Dirty tracking (delegated to tab-integration.js) ──────────────────────────
...
cmEl.addEventListener("dragover", (e: DragEvent) => {
  e.preventDefault();
  e.dataTransfer!.dropEffect = "copy";
});
cmEl.addEventListener("drop", (e: DragEvent) => {
  const file = e.dataTransfer!.files && e.dataTransfer!.files[0] as (File & { path?: string }) | undefined;
...
cmEl.addEventListener("paste", async (e: ClipboardEvent) => {
  const imageFiles = Array.from(e.clipboardData?.items || [] as unknown as DataTransferItemList)
    .filter((item) => (item as DataTransferItem).kind === "file" && (item as DataTransferItem).type.startsWith("image/"))
  ...
  if (!tab?.path) {
    if (status) status.textContent = "Save the Markdown file before pasting images";
    return;
  }

  const assets: { altText: string; markdownPath: string }[] = [];
  for (const file of imageFiles) {
    try {
      assets.push(await saveImageAsset(tab.path, file));
    } catch (err: any) {
      if (status) status.textContent = "Failed to paste image: " + err.message;
      return;
    }
  }

  if (!assets.length) {
    if (status) status.textContent = "Clipboard image could not be read";
    return;
  }
  ...
  if (status) status.textContent = assets.length === 1
...
busOn("file-saved", ({ file, name }: { file: string; name: string }) => {
...
function newFromTemplate(): void {
  hideWelcome();
  openTab("", "Untitled", BEORN_TEMPLATE, 0);
  cm.focus();
  triggerRender();
}
...
const shortcutActions: Record<string, unknown> = {
...
pagedReady
  .then(async () => {
    ...
    // Wire chat sidebar
    setGetConversation((key: string) => getConversation(key));

    onChatSend((agentKey: string, prompt: string, context: any) => {
      const selection = context || { text: "", lineStart: 0, lineEnd: 0 };
      sendRequest(agentKey, prompt, selection);
      refreshChat();
    });

    onChatAnswer((agentKey: string, questionId: string, value: string) => {
    ...
    if (sparkBtn) {
      ...
      sparkBtn.onclick = () => {
        ...
        if (!text) return;
        const context = { text, lineStart: from.line, lineEnd: to.line, label: null as string | null };
