...
export function wireElectronMenus(api: { on?: (channel: string, callback: (...args: unknown[]) => void) => void } | null | undefined, actions: Record<string, any>): void {
  if (!api?.on) return;
  ...
  api.on("open-file-path", (filePath: unknown) => openFilePath(filePath as string));
  api.on("open-folder-path", (folderPath: unknown) => openFolderByPathAndLoad(folderPath as string));
