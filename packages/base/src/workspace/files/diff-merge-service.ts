...
const splitLines = (t: string): string[] => t === "" ? [] : t.split("\n");


// ── Diff algorithm (Myers-like, minimal) ──────────────────────────
export function computeDiff(oldLines: string[], newLines: string[]): Array<{ type: string; text: string; oldLine?: number; newLine?: number }> {
...
function renderWordDiff(oldLine: string, newLine: string, cls: string): string {
...
export function renderDiffHtml(diff: Array<{ type: string; text: string; oldLine?: number; newLine?: number }>): string {
  ...
  let html = "";
  let lastShown = -1;
  diff.forEach((d: { type: string; text: string; oldLine?: number; newLine?: number }, i: number) => {
...
export function threeWayMerge(baseText: string, localText: string, remoteText: string): { text: string; hasConflicts: boolean } {
...
function diffToEditMap(baseLines: string[], modifiedLines: string[]): Map<number, { oldCount: number; newLines: string[] }> {
...
export let gdModifiedTime: string | null = null; // modifiedTime from Drive when file was loaded
let conflictResolve: ((action: string) => void) | null = null; // promise resolver for conflict modal
...
export function resolveConflict(action: string): void {
  document.getElementById("diffModal")?.classList.remove("open");
  if (conflictResolve) { conflictResolve(action); conflictResolve = null; }
}

export function showDiffModal(localText: string, remoteText: string, fileName: string, hint: string | undefined): Promise<string> {
  ...
  if (titleEl) titleEl.textContent = "Conflict — " + fileName;
  if (hintEl) hintEl.textContent = hint ||
    "This file was modified externally since you loaded it. Lines prefixed with − are your version, + are the disk version.";
  if (containerEl) containerEl.innerHTML = renderDiffHtml(diff);
  if (modalEl) modalEl.classList.add("open");
  return new Promise<string>(resolve => { conflictResolve = resolve; });
}
