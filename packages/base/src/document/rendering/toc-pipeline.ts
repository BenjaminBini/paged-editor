...
async function getMemoireSections(): Promise<Array<{ name: string; markdown: string }>> {
...
export async function buildTocRenderResult({ assetBaseHref = "", folderPath = getFolderPath() } = {}): Promise<{ sectionHtml: string; headerText: string; language: string; rootPageName: string; lineNumberOffset: number; lineStarts: number[]; sourceBlocks: unknown[] }> {
  const sections = await getMemoireSections();
  const headings: Array<{ depth: number; id: string; title: string; num: number | null; colorPair: [string, string] }> = [];
