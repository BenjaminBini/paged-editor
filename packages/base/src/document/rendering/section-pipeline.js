...
const STANDALONE_PAGE_BREAK_RE = /^[ \t]*(?:\\newpage|\/newpage)[ \t]*$/;
const IMAGE_ALIGNMENT_VALUES = new Set(["left", "center", "right"]);
const IMAGE_MAX_WIDTH_RE = /^\d+(?:\.\d+)?(?:px|%|em|rem|vw|vh|vmin|vmax|svw|svh|lvw|lvh|dvw|dvh|cm|mm|in|pt|pc|ch|ex)$/i;
...
function isStandalonePageBreak(value = "") {
  return STANDALONE_PAGE_BREAK_RE.test(String(value || "").trim());
}

function getTableText(token) {
  const rows = [
...
function getListText(token) {
  return (token.items || []).map((item) => item.text || "").join("\n");
}

function buildSourceBlocks(tokens) {
  const blocks = [];
  let offset = 0;
...
function renderImageTag(parser, token) {
  const text = token.tokens
    ...
    : token.text || "";
  const options = parseImageOptions(token.href || "");
