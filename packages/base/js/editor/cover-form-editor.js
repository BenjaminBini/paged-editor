...
export function populateCoverForm(jsonString) {
  ...
  for (const key of LOGO_KEYS) {
    ...
    r.file = String(entry.file || "");
    r._dataUrl = null;
    r.showInCover.checked = entry.showInCover ?? false;
    r.coverWidth.value = entry.coverWidth ?? 180;
    r.coverX.value = entry.coverX ?? 0;
    r.coverY.value = entry.coverY ?? 0;
    r.showInFooter.checked = entry.showInFooter ?? false;
    r.footerWidth.value = entry.footerWidth ?? 80;
    r.footerX.value = entry.footerX ?? 0;
    r.footerY.value = entry.footerY ?? 0;
...
export function syncCoverFormToEditor() {
  _syncing = true;
  try {
    cm.setValue(serializeCoverForm());
  } finally {
    _syncing = false;
  }
