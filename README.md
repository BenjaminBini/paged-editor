...
For web mode, `js/web-api.js` provides a drop-in replacement for `window.electronAPI` backed by REST `fetch()` calls. It loads as a regular `<script>` (not a module) so it executes before the ES module graph evaluates. The rest of the editor code is unchanged.

```
Electron mode:
  app.js → window.electronAPI → IPC → main.js → Node fs

Web mode:
  app.js → window.electronAPI (web-api.js shim) → fetch → Express server → Node fs
```
...
| File | Role |
|------|------|
| `js/web-api.js` | REST-backed `window.electronAPI` shim for web mode |
