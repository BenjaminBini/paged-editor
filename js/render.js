...
let _headingCollector = null; // array to collect headings for TOC (null = disabled)
...
function slugify(text) {
  return text.replace(/<[^>]+>/g, '').replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '-').toLowerCase();
}
...
marked.use({
  renderer: {
    heading(token) {
      ...
      // Generate heading id for TOC linking
      const plainText = text.replace(/<[^>]+>/g, '');
      const hid = `h-${_headingIdCounter++}-${slugify(plainText)}`;
      const idAttr = ` id="${hid}"`;
      ...
      if (depth === 1) {
        ...
        const title = _partieNum
          ? `Partie ${_partieNum} ${disc} ${stripped}`
          : clean;
        if (_headingCollector) {
          const tocTitle = decodeEntities((stripped || clean).replace(/<[^>]+>/g, ''));
          _headingCollector.push({ depth: 1, id: hid, title: tocTitle, num: _partieNum || null, colorPair: pair });
        }
        return `<h1${idAttr}${sl} data-color-index="${_colorIdx % 5}" style="color:${primary};${vars}">${title}${buildUnderline(pair)}</h1>\n`;
      }
      ...
      // H2/H3 not collected for TOC (only H1 partie headings)
...
export async function buildPagedHtml(md) {
  ...
  // Detect color from markdown H1 or filename (same logic as triggerRender)
...
// ── Cover & Sommaire HTML builder ───────────────────────────────────────────

function buildCoverHtml(fm) {
  const title = fm.title || "Document";
  const doctype = fm.doctype || "Mémoire technique";
...
function buildSommaireHtml(headings) {
  // headings: [{ depth, id, title, num, colorPair }]
...
export async function buildFullMemoireHtml(sections) {
  // sections: [{ name, markdown }] in sidebar order
  // Use frontmatter from the first section for header/language
  const first = sections[0];
  const { fm } = parseFrontmatter(first.markdown);
  const headerText = buildHeaderText(fm);
  const language = fm.language || "fr";

  // Parse all sections with heading collection enabled
  const allHeadings = [];
  ...
  for (const sec of sections) {
    ...
    // Detect partie/color from markdown H1 or filename prefix
    const pm = body.match(/^#\s+Partie\s+(\d+)/im);
    let partieNum;
    if (pm) {
      partieNum = parseInt(pm[1], 10);
    ...
    // Temporarily set active filename for heading renderer
    const prevFileName = _activeFileName;
    _activeFileName = sec.name;

    // Enable heading collector for this section
    _headingCollector = allHeadings;

    let html = parseMarkdownSync(body, ci, 0);
    const queue = [..._mermaidQueue];
    html = await resolveMermaid(html, queue);

    _headingCollector = null;
    _activeFileName = prevFileName;
  ...
  // Build cover + sommaire + all sections
